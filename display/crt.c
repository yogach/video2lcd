...
static int CRTDeviceShowPage ( PT_PixelDatas ptPixelDatas );
...
//	vga_setcolor(4);
	...
	for ( iX = 0; iX < 320; iX++ )
	{
		for ( iY = 0; iY < 200; iY++ )
		{
			gl_setpixelrgb ( iX, iY, iRed, iGreen, iBlue );    //vga_drawpixel(iX, iY);
		}
	}
	gl_copyscreen ( physicalscreen );
...
{
	int x,y;
	unsigned int* pwdSrc = ( unsigned int* ) ptPixelDatas->aucPixelDatas;
	unsigned int dwColor;
	unsigned int dwRed, dwGreen, dwBlue;

	if ( ptPixelDatas->iBpp != 32 )
	{
		DBG_PRINTF ( "can't disp this : %d bpp\r\n",ptPixelDatas->iBpp );
		return -1;
	}
